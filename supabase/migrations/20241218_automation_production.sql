...
-- Self-contained migration (creates base tables if not exist)

-- =====================================================
-- 0. Create base automation_rules table (if not exists)
-- =====================================================
...
-- Create automation execution log (if not exists)
...
-- Base RLS Policies (if not exists)
ALTER TABLE automation_rules ENABLE ROW LEVEL SECURITY;
ALTER TABLE automation_logs ENABLE ROW LEVEL SECURITY;

DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'automation_rules' AND policyname = 'Users can view own rules') THEN
    CREATE POLICY "Users can view own rules" ON automation_rules FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'automation_rules' AND policyname = 'Users can create own rules') THEN
    CREATE POLICY "Users can create own rules" ON automation_rules FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'automation_rules' AND policyname = 'Users can update own rules') THEN
    CREATE POLICY "Users can update own rules" ON automation_rules FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'automation_rules' AND policyname = 'Users can delete own rules') THEN
    CREATE POLICY "Users can delete own rules" ON automation_rules FOR DELETE USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'automation_logs' AND policyname = 'Users can view own logs') THEN
    CREATE POLICY "Users can view own logs" ON automation_logs FOR SELECT USING (auth.uid() = user_id);
  END IF;
END $$;

-- Base indexes (if not exists)
